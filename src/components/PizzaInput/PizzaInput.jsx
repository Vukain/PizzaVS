    ...
    return (
        <div className={`pizza_input pizza_input--${props.name}`}>
            ...
            <form className='pizza_input__form' action=''>
                <input title='diameter of one pizza' placeholder='DIAMETER' aria-label={`pizza ${props.name} diameter`} min='1' onKeyDown={(event) => ['e', 'E', '-'].includes(event.key) && event.preventDefault()} onBlur={validationHandler} className={`pizza_input__input pizza_input__input--${props.name}`} type="number" onChange={({ target }) => { setPizzaSize(parseInt(target.value)) }} />
                <input title='number of pizzas' placeholder='COUNT' aria-label={`pizza ${props.name} count`} min='1' onKeyDown={(event) => ['e', 'E', '-'].includes(event.key) && event.preventDefault()} onBlur={validationHandler} className={`pizza_input__input pizza_input__input--${props.name}`} type="number" onChange={({ target }) => { setPizzaCount(parseInt(target.value)) }} />
                <input title='total price for this variant' placeholder='PRICE' aria-label={`pizza ${props.name} price`} min='1' onKeyDown={(event) => ['e', 'E', '-'].includes(event.key) && event.preventDefault()} onBlur={validationHandler} className={`pizza_input__input pizza_input__input--${props.name}`} type="number" onChange={({ target }) => { setPizzaPrice(parseInt(target.value)) }} />
            </form>
